...
<div class="w-full h-[900px] flex flex-col justify-center items-center">
  <div class="w-[80%] h-[10%] flex justify-start items-center">
...
<table class="w-[80%] min-h-[50%] border-[1px] border-[#A9A9A9]">
  <thead>
  ...
  <tbody>
    <item v-for="(item , index) in store.listData" :key="item.id" :index="index" @index="handGetIndex" :item="item"  @idItem="handleEventId" />
  </tbody>
</table>
<Model v-if="store.model"  :id="idData" @update="handUpdate" />
</div>
</template>
<script setup>
import {ref } from "vue";
